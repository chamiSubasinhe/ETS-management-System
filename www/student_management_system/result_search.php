...
<body class="theme-red">
	...
	<div class="row clearfix">
			...
			<?php
				...
				if(isset($_POST["search"])){
					...
					if(isset($_POST["option"])){
						if($_POST["option"]==1){
								$query=mysqli_query($conn,"select * from result WHERE 	examID LIKE '%".$search."%'");
						}
					}
					
					if(isset($_POST["option"])){
						if($_POST["option"]==2){
								$query=mysqli_query($conn,"select * from result WHERE stuID LIKE '%".$search."%'");
						}
					}
					
					if(isset($_POST["option"])){
						if($_POST["option"]==3){
								$query=mysqli_query($conn,"select * from result WHERE marks LIKE '%".$search."%'");
						}
					}
					
					if(isset($_POST["option"])){
						if($_POST["option"]==4){
								$query=mysqli_query($conn,"select * from result WHERE moduleID LIKE '%".$search."%'");
						}
					}
					
					if(isset($_POST["option"])){
						if($_POST["option"]==5){
								$query=mysqli_query($conn,"select * from result R, module M WHERE M.moduleID LIKE '%".$search."%'");
						}
					}
		...
		while($row2=mysqli_fetch_array($query)){
			...
			if($row2['marks']>=90){
				$grade = 'A+';
			}else if($row2['marks']>=85){
				$grade = 'A';
			}else if($row2['marks']>=80){
				$grade = 'A-';
			}else if($row2['marks']>=75){
				$grade = 'B+';
			}else if($row2['marks']>=70){
				$grade = 'B';
			}else if($row2['marks']>=65){
				$grade = 'B-';
			}else if($row2['marks']>=60){
				$grade = 'C+';
			}else if($row2['marks']>=55){
				$grade = 'C';
			}else if($row2['marks']>=50){
				$grade = 'C-';
			}else if($row2['marks']<35){
				$grade = 'F';
			}else if($row2['marks']==null){
				$grade = 'No Grade';
			}
			?>
									<tr>
