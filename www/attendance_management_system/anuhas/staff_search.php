                        ...
                        <div class="body">
                            <form id="form_validation"  action="" method="POST">
							 ...
							 <div class="demo-radio-button">Search: 
                                ...
                                <label for="radio_7">By Staff ID</label>
							
								<input type="radio" id="radio_8" value="2" name="option" class="radio-col-pink"  />
                                <label for="radio_8">By Present</label>
			...
			<?php
				...
				if(isset($_POST["search"])){
					...
					if(isset($_POST["option"])){
						if($_POST["option"]==1){
								$query=mysqli_query($conn,"select * from staffattendence WHERE staffID LIKE '%".$search."%'");
						}
					}
					
					if(isset($_POST["option"])){
						if($_POST["option"]==2){
								$query=mysqli_query($conn,"select * from staffattendence WHERE present LIKE '%".$search."%'");
						}
					}
					
					if(isset($_POST["option"])){
						if($_POST["option"]==3){
								$query=mysqli_query($conn,"select * from staffattendence WHERE date LIKE '%".$search."%'");
						}
					}
					
					if(isset($_POST["option"])){
						if($_POST["option"]==4){
								$query=mysqli_query($conn,"select * from staffattendence WHERE arrivalTime LIKE '%".$search."%'");
						}
					}
					
					if(isset($_POST["option"])){
						if($_POST["option"]==5){
								$query=mysqli_query($conn,"select * from staffattendence WHERE depatureTime LIKE '%".$search."%'");
						}
					}
		...
		while($row2=mysqli_fetch_array($query)){
			?>
									<tr>
                                            ...
                                            <td><?php echo $row2['arrivalTime']; ?></td>
