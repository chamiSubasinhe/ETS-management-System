...
<!-- Exportable Table -->
	...
	$arr = array();
			...
			$tot=0;
		$query1=mysqli_query($conn,"select count(inquaryID) as 'id' from inquiry WHERE MONTH(date) BETWEEN 1 AND 12 AND YEAR(date) = YEAR(CURRENT_DATE) GROUP BY MONTH(date) ORDER BY MONTH(date)");
