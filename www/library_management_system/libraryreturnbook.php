...
if(isset($_POST['return'])){
	if(empty($_POST['isbn']) || empty($_POST['id']) || empty($_POST['date'])){
		...
		if(!preg_match('/^[0-9]{13}$/',$isbn) || !preg_match('/^[A-Z]{6}[0-9]{6}$/',$id)){
			$banner2 = "Return Operation Unsuccessful - Invalid ISBN/MemberID";
			$flag3 = 1;
			...
			$datediff = strtotime($currentDate) - strtotime($date);
			$fine = round($datediff / (60 * 60 * 24)) * 50;
			$query = "UPDATE libraryborrow SET dateReturned = '$currentDate',fine = '$fine' WHERE isbn = '$isbn' AND memberId = '$id' AND dateBorrowed = '$date' ";
			$result = mysqli_query($conn, $query);		
		
			
			if($result > 0){
				$banner2 = "Return Operation Successful";
				$flag2 = 1;
			}
			else{
				$banner2 = "Return Operation Unsuccessful";
				$flag3 = 1;
			}
		}	
        ...
        <div class="container-fluid">
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
						...
						<div class = "body">
						<h2 class="card-inside-title">Return</h2>
						...
						<?php if($flag == 0 && $flag2 == 0 && $flag3 == 1){echo '<div class="alert bg-red">' .$banner2. '</div>';} ?>
							...
							<div class = "col-sm-4">
								<input type = "submit" class="btn bg-red btn-block waves-effect"  name = "return" value = "RETURN"/>
							</div>
						</form>
						</div>
